...
function initCustomerMessagesSection() {
    console.log('✅ تهيئة قسم رسائل العملاء - بدء');
    ...
    if (!grid || !tpl) {
        console.error('❌ عناصر DOM غير موجودة:', { grid, tpl });
        return;
    }

    emptyStateManager.createEmptyState('customer-messages');
    ...
    function getContactType(contact) {
        ...
        // تحقق إذا كان بريد إلكتروني
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (emailRegex.test(cleanContact)) {
            return 'email';
        }
        
        // تحقق إذا كان رقم هاتف (يحتوي على أرقام فقط أو برقم دولي)
        const phoneRegex = /^[\+]?[0-9\s\-\(\)]{8,}$/;
        if (phoneRegex.test(cleanContact.replace(/[\s\-\(\)]/g, ''))) {
            return 'phone';
        }
    ...
    // تنظيف وتحضير رقم الهاتف لواتساب
    function preparePhoneNumber(phone) {
        ...
        // إضافة رمز الدولة إذا لم يكن موجوداً (افتراضي مصر +20)
    ...
    // إنشاء رابط الرد المناسب حسب نوع الاتصال
    ...
    onValue(messagesRef, (snapshot) => {
        console.log('📨 بيانات الرسائل المستلمة:', snapshot.val());
    ...
    function renderMessages() {
        ...
        if (filteredMessages.length === 0) {
            if (Object.keys(messagesData).length === 0) {
                emptyStateManager.showEmptyState('customer-messages');
            } else {
                emptyStateManager.showSearchEmptyState('customer-messages');
            }
            return;
        } else {
            emptyStateManager.hideEmptyState('customer-messages');
        }
        ...
        filteredMessages.forEach(([key, m]) => {
            ...
            if (!cardEl) {
                console.error('❌ عنصر .message-card-new غير موجود في القالب');
                return;
            }
            ...
            if (contactEl) {
                ...
                contactEl.title = displayContact;
            }
            ...
            if (statusEl) {
                ...
                statusEl.className = `msg-status status-${m.status}`;
            }
            ...
            if (previewEl) {
                if (m.message && m.message.length > maxChars) {
                    const truncated = m.message.slice(0, maxChars).trim();
                    previewEl.textContent = truncated + '...';
                } else {
                    previewEl.textContent = m.message || 'لا توجد رسالة';
                }
                previewEl.setAttribute('data-full', m.message || '');
            }

            // أزرار البطاقة - الحفاظ على نفس التنسيق
            const replyBtn = node.querySelector('.reply-btn');
            const deleteBtn = node.querySelector('.delete-msg');
            
            // زر الرد - نفس التنسيق ولكن بوظيفة ذكية
            if (replyBtn) {
                // الحفاظ على نفس التنسيق الأصلي
                replyBtn.innerHTML = '<i class="fas fa-reply"></i> رد';
                replyBtn.className = 'btn btn-wa reply-btn';
        ...
        console.log('✅ تم عرض الرسائل:', filteredMessages.length);
    }
    ...
    function openMessageModal(key, message) {
        ...
        // تحديث حالة أزرار المودال
        if (modalMarkRead) {
            if (message.status === 'replied') {
                modalMarkRead.innerHTML = '<i class="fas fa-check-circle"></i> مقروءة';
                modalMarkRead.disabled = true;
            } else {
                modalMarkRead.innerHTML = '<i class="fas fa-check"></i> تم الرد';
                modalMarkRead.disabled = false;
            }
            
            // إضافة مستمعي الأحداث لأزرار المودال
            modalMarkRead.onclick = () => {
                markMessageAsReplied(key);
            };
        }
    ...
    function markMessageAsReplied(key) {
        if (messagesData[key] && messagesData[key].status !== 'replied') {
            update(ref(db, `customerMessages/${key}`), { status: 'replied' })
                .then(() => {
                    ...
                    // إذا كانت المودال مفتوحة، قم بتحديثها
                    if (currentMessageKey === key && modalMarkRead) {
                        modalMarkRead.innerHTML = '<i class="fas fa-check-circle"></i> مقروءة';
                        modalMarkRead.disabled = true;
                    }
                })
                .catch(error => {
                    ...
                    utils.showToast('حدث خطأ أثناء التعديل: ' + error.message, 'error');
                });
        }
    }

    function deleteMessage(key) {
        remove(ref(db, `customerMessages/${key}`))
            .then(() => {
                ...
                // إذا كانت الرسالة المحذوفة هي المعروضة في المودال، أغلق المودال
    ...
    if (searchInput) {
        // يمكن إضافة بحث عند الضغط على Enter إذا لزم الأمر
    }

    console.log('✅ قسم رسائل العملاء جاهز للاستخدام');
}
